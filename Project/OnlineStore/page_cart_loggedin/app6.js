...
function showShoppingList(arr) {
  ...
  arr.forEach((element, key) => {
    ...
    let btn = `<a href='../page_details/Page-Details.html?id=${element.id}'><button>${element.name}</button></a>`;
...
let existentdiv = document.getElementById("btn-styled");
function checkout(el) {
  if (el.length != 0) {
    let button_checkout = document.createElement('div');
    button_checkout.id = "check";
    button_checkout.innerHTML = `
    <button id = "checkout" type="submit" onclick="order_submitted()">>> Submit Order</button>`
    existentdiv.appendChild(button_checkout)
  }
}
checkout(finalEntries)
