document.getElementById("myFormCreate").addEventListener("submit", async (e) => {
  e.preventDefault();
  ...
  try {
    const response = await fetch("/checkSignup", {
      method: "POST",
      headers: {
    ...
    if(data.found === true && name=="admin" && password=="1@1@") {
      ...
      banner.innerHTML = `
        <span> User already exists. Please go back to the login page.</span>`
